...
export function createMaze(width: number, height: number): Maze {
	const maze: Maze = Array.from(Array(height), (_y, y) =>
		Array.from(Array(width), (_x, x) => ({
			location: { x, y },
			visited: false,
			walls: Object.fromEntries(MazeDirections.map((direction) => [direction, true])) as Record<
				MazeDirection,
				boolean
			>
		}))
	...
	while (visitedCells.size < width * height) {
		...
		let currentCell =
			firstCellOptions[randInt(firstCellOptions.length)] ?? maze[randInt(height)][randInt(width)];
